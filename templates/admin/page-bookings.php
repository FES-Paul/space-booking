...
?>
<style>
.sb-admin-bookings { font-family: -apple-system,BlinkMacSystemFont,"Segoe UI",Roboto; }
.sb-tabs { display: flex; border-bottom: 1px solid #c3c4c7; margin: 0 0 20px; }
.sb-tab-btn { background: none; border: none; padding: 12px 24px; cursor: pointer; font-size: 14px; color: #50575e; border-bottom: 2px solid transparent; }
.sb-tab-btn.active { color: #1d2327; border-bottom-color: #2271b1; }
.sb-tab-content { display: none; }
.sb-tab-content.active { display: block; }
.sb-calendar-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(160px, 1fr)); gap: 12px; }
.sb-day-cell { border: 1px solid #e0e3e6; border-radius: 6px; padding: 12px; background: #fff; }
.sb-day-header { font-weight: 600; margin: 0 0 8px; color: #1d2327; }
.sb-booking { background: #f6f7f7; border-radius: 4px; padding: 6px 8px; margin: 4px 0; font-size: 13px; }
.sb-status { padding: 2px 6px; border-radius: 3px; font-size: 11px; font-weight: 500; text-transform: uppercase; }
.sb-status--confirmed { background: #d4edda; color: #155724; }
.sb-status--pending { background: #fff3cd; color: #856404; }
	.sb-month-section { margin-bottom: 40px; }
	.sb-month-section h3 { margin: 0 0 20px; color: #1d2327; border-bottom: 2px solid #2271b1; padding-bottom: 8px; }

.sb-filters { background: #fff; border: 1px solid #c3c4c7; padding: 16px; border-radius: 6px; margin-bottom: 24px; }
.sb-filters select, .sb-filters input { margin-right: 12px; padding: 6px 10px; border: 1px solid #8c8f94; border-radius: 3px; }
@media (max-width: 782px) { .sb-calendar-grid { grid-template-columns: repeat(3, 1fr); } }
</style>
...
<div class="sb-admin-bookings">
	...
	<form method="get" class="sb-filters">
		...
		<input type="date" name="date_from" value="<?php echo esc_attr( $_GET['date_from'] ?? '' ); ?>" placeholder="From">
	...
	<div class="sb-calendar-grouped">
		...
		if ( $date_from = sanitize_text_field( $_GET['date_from'] ?? '' ) ) {
			$where[] = 'b.booking_date >= %s';
			$params[] = $date_from;
		}
		if ( $date_to = sanitize_text_field( $_GET['date_to'] ?? '' ) ) {
			$where[] = 'b.booking_date <= %s';
			$params[] = $date_to;
		}
		$where[] = 'b.booking_date >= CURDATE() AND b.booking_date < DATE_ADD(CURDATE(), INTERVAL 12 MONTH)';
