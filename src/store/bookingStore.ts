...
interface BookingState {
  ...
  setCheckoutData:  (data: { clientSecret: string; bookingId: number; totalPrice: number; breakdown: PriceBreakdownItem[] }) => void;
  setPriceBreakdown: (breakdown: PriceBreakdownItem[], total: number) => void;
  confirmBooking:   () => void;
  reset:            () => void;
}

const DEFAULT_CUSTOMER: CustomerInfo = { name: '', email: '', phone: '', notes: '' };
...
export const useBookingStore = create<BookingState>((set, get) => ({
  ...
  nextStep: () => set((s) => ({ currentStep: Math.min(s.currentStep + 1, 6) as BookingStep })),
  prevStep: () => set((s) => ({ currentStep: Math.max(s.currentStep - 1, 1) as BookingStep })),
  ...
  // ── Step 2 ───────────────────────────────────────────────────────────────
  setDate:      (date) => set({ selectedDate: date, selectedStartTime: '', selectedEndTime: '', selectedExtras: [] }),
  setStartTime: (time) => set({ selectedStartTime: time, selectedEndTime: '', selectedExtras: [] }),
  ...
  // ── Step 5 ───────────────────────────────────────────────────────────────
  setCheckoutData: ({ clientSecret, bookingId, totalPrice, breakdown }) =>
    set({ clientSecret, bookingId, totalPrice, priceBreakdown: breakdown }),
