...
export function Step3Addons() {
  const {
    ...
    setAvailableExtras, setPriceBreakdown,
    nextStep, prevStep,
  } = useBookingStore();
  ...
  const [preview, setPreview] = useState<{ total: number; breakdown: { label: string; amount: number }[] } | null>(null);

  const spaceId   = selectedSpace?.id ?? selectedPackage?.space_id ?? 0;
  const packageId = selectedPackage?.id;

  useEffect(() => {
    if (!spaceId || !selectedDate || !selectedStartTime || !selectedEndTime) return;
    setLoading(true);
    fetchExtras(spaceId, selectedDate, selectedStartTime, selectedEndTime)
      .then((data) => { setExtras(data); setAvailableExtras(data); })
  ...
  }, [selectedExtras, spaceId, selectedDate, selectedStartTime, selectedEndTime, packageId]);

  const isSelected = (id: number) => selectedExtras.some((e) => e.extra_id === id);
  ...
  return (
    <div className="sb-step sb-step-3">
            ...
            >
              <div className="sb-extra-card__info">
                ...
                <div>
                  ...
                  <span className="sb-extra-card__price">${extra.price.toFixed(2)}</span>
                  {!extra.is_available && (
                    <span className="sb-badge sb-badge--sold-out">Sold Out</span>
                  )}
      ...
      <div className="sb-step__actions">
        ...
        <button className="sb-btn sb-btn--primary" onClick={nextStep}>Continue →</button>
      </div>
    </div>
  );
}
